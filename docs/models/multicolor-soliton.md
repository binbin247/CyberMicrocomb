...
$\eta$ 是 XPM 系数，$\gamma$ 是 FWM 耦合系数，$\Delta_s$ 和 $\Delta_i$
描述 signal/idler 相对 primary 的 FSR mismatch。默认参数来自 supplementary
simulation parameters 的损耗归一化估计。
...
- `Intracavity energy`：看三场能量是否先后建立并达到稳定。
